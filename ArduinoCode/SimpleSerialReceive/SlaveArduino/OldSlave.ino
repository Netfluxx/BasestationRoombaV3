...
void executeCommand(byte command){
  switch (command){
    case CMD_FORWARD:
      ...
      analogWrite(IN2, 255); // Full speed forward
      break;
    case CMD_BACKWARD:
      digitalWrite(IN2, LOW);
      analogWrite(IN1, 255); // Full speed backward
      break;
    case CMD_RIGHT:
      digitalWrite(IN1, LOW);
      analogWrite(IN2, 128); // Half speed forward (for turning right)
      break;
    case CMD_LEFT:
      digitalWrite(IN2, LOW);
      analogWrite(IN1, 128); // Half speed backward (for turning left)
      break;
    case CMD_STOP:
      digitalWrite(IN1, LOW);
